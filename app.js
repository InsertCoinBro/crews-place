...
async function boot() {
  ...
  if (
    import.meta.env.DEV &&
    new URLSearchParams(location.search).has("space-tube-preview")
  ) {
    game.start();
  ...
  if (
    import.meta.env.DEV &&
    new URLSearchParams(location.search).has("space-tube-test")
  ) {
    import("./tests/space-tube-browser-checks.js").then((m) =>
      m.runSpaceTubeChecks(game),
    );
  }
  if (
    import.meta.env.DEV &&
